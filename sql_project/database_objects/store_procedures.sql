...
BEGIN
    ...
    SELECT precio INTO v_precio FROM estudio WHERE nombre = p_estudio;
    IF v_precio IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Estudio no encontrado.';
    END IF;
    ...
    -- Crear un nuevo pago con el precio obtenido y el tipo de pago especificado
    ...
    INSERT INTO turno (fecha_y_hora, dni_paciente, estudio, resultado, id_tecnico) 
    VALUES (CONCAT(DATE(p_fecha_turno), ' ', TIME(p_fecha_turno)), p_dni_paciente, p_estudio, NULL, p_id_tecnico);
...
CALL crear_talon(1, 20, 12345678, 'Amilasa', 5, '2024-09-01 15:00:00', 'tarjeta debito');
